...
1.  How to generat the Fig.2 (the Results of the training process in each episode)
...
2. How to generat the figures for both the Fig.3 (profit of the cooperative obtained with PDRL and heuristic searching) and Fig.4 (Gantt Chart)
(1) In Parameter.py, configure parameters as follows:
...
3. How to generat the Fig.5 (Agricultural knowledge acquired through the PDRL algorithm)
(1) In Parameter.py, configure parameters as follows:
SamePolicyDifferetPro = True #if True, comparing Cooperatives Adopting Different Strategies
...
4. How to generat the Fig.6 ( Dynamic planning process using PRDL)
(1) In Parameter.py, configure parameters as follows:
SamePolicyDifferetPro = True #if True, comparing Cooperatives Adopting Different Strategies
ConsiderMiniSupply = True # if True, considering Diversity (Market) Constraint
